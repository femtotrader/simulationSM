...
# args: sl 0.0011 tp 0.0015 month 04 year 2015
...
def make_list(number_files):
    ...
    ss.intersection(fs)
    ss.union(fs)
    lista = ss.union(fs) - ss.intersection(fs)
...
#    print row, dbar.bid.ix[row], dbar.index[row]
        ...
        if inTime == True:
            ...
            dbar_1h["rsi"] = RSI(dbar_15min["bid"],21)          
            dbar_1h['ra20'] = pd.rolling_mean(dbar_15min.bid, 20)             
            dbar_1h['ra50'] = pd.rolling_mean(dbar_15min.bid, 50)
            ...
            tradeBuy = True
            f = open(OUTFILE, 'a')
        while (tradeSell == True):
            rowTemp += 1
            priceTempBid = (float(dbar.bid[rowTemp]))
            priceTempAsk = (float(dbar.ask[rowTemp]))
            dSell = priceTempAsk - priceBid
            if (dSell > SL): 
                f.write("SELL_PERDANT,%s,%s,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f\n" %(Timestamp(dbar.index[row]), Timestamp(dbar.index[rowTemp]), float(priceBid), float(priceTempAsk), float(dbar_1h.rsi[-1]), float(dbar_1h.rsi[-2]), float(dbar_1h.rsi[-3]), float(dbar_1h.ra50[-1]), float(dbar_1h.ra20[-1]), float(dbar_15min.rsi[-1]), float(dbar_15min.rsi[-2]), float(dbar_15min.rsi[-2]), float(dbar_15min.ra50[-1]), float(dbar_15min.ra20[-1]), float(dbar_5min.rsi[-1]), float(dbar_5min.ra50[-1]), float(dbar_5min.ra20[-1])))
                tradeSell = False
            if (dSell  <= -1 * TP): 
                f.write("SELL_GAGNANT,%s,%s,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f\n" %(Timestamp(dbar.index[row]), Timestamp(dbar.index[rowTemp]), float(priceBid), float(priceTempAsk), float(dbar_1h.rsi[-1]), float(dbar_1h.rsi[-2]), float(dbar_1h.rsi[-3]), float(dbar_1h.ra50[-1]), float(dbar_1h.ra20[-1]), float(dbar_15min.rsi[-1]), float(dbar_15min.rsi[-2]), float(dbar_15min.rsi[-2]), float(dbar_15min.ra50[-1]), float(dbar_15min.ra20[-1]), float(dbar_5min.rsi[-1]), float(dbar_5min.ra50[-1]), float(dbar_5min.ra20[-1])))
                tradeSell = False
        ...
        while (tradeBuy == True):
            ...
            if (dBuy < (-1 * SL)): 
                f.write("BUY_PERDANT %s %s %f %f %f %f %f %f %f %f\n" %(Timestamp(dbar.index[row]), Timestamp(dbar.index[rowTemp]), float(priceBid), float(priceTempAsk), float(dbar_1h.rsi[-1]), float(dbar_1h.rsi[-2]), float(dbar_1h.rsi[-3]), float(dbar_1h.ra50[-1]), float(dbar_1h.ra20[-1]), float(dbar_15min.rsi[-1]), float(dbar_15min.rsi[-2]), float(dbar_15min.rsi[-2]), float(dbar_15min.ra50[-1]), float(dbar_15min.ra20[-1]), float(dbar_5min.rsi[-1]), float(dbar_5min.ra50[-1]), float(dbar_5min.ra20[-1])))
                tradeBuy = False
            if (dBuy >= TP): 
                f.write("BUY_GAGNANT %s %s %f %f %f %f %f %f %f %f\n" %(Timestamp(dbar.index[row]), Timestamp(dbar.index[rowTemp]), float(priceBid), float(priceTempAsk), float(dbar_1h.rsi[-1]), float(dbar_1h.rsi[-2]), float(dbar_1h.rsi[-3]), float(dbar_1h.ra50[-1]), float(dbar_1h.ra20[-1]), float(dbar_15min.rsi[-1]), float(dbar_15min.rsi[-2]), float(dbar_15min.rsi[-2]), float(dbar_15min.ra50[-1]), float(dbar_15min.ra20[-1]), float(dbar_5min.rsi[-1]), float(dbar_5min.ra50[-1]), float(dbar_5min.ra20[-1])))
                tradeBuy = False
        row+=1
